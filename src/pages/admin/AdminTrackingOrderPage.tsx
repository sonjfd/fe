...
const formatDateWithWeekday = (iso: string) => {
  ...
  const day = d.getDay();
  const weekday = day === 0 ? "Chủ nhật" : `Thứ ${day + 1}`; // Thứ 2–7
...
const AdminGhnTrackingPage: React.FC = () => {
  const { ghnOrderCode } = useParams<{ ghnOrderCode: string }>();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const d = await getGhnOrderDetail(ghnOrderCode);
        setData(d);
      } catch (e: any) {
        console.error(e);
        toast.error(
          e?.response?.data?.message || "Không tải được tracking GHN"
        );
      } finally {
        setLoading(false);
      }
  ...
  // group theo ngày nhưng dùng data.timeline
  ...
  src.forEach((log: any) => {
    ...
    // ✅ mới: dùng ngày LOCAL (múi giờ của browser)
  ...
  if (loading || !data) {
    return (
      <div className="p-6 text-sm text-slate-600">
        Đang tải thông tin vận đơn GHN...
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold">
            Theo dõi đơn GHN – {info.order_code}
          </h1>
          <button
            onClick={() => navigate(-1)}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          {/* Thông tin chi tiết */}
          <section className="bg-white rounded-xl shadow-sm p-4 text-sm">
            <h2 className="font-semibold mb-3">Người gửi</h2>

            <div className="grid grid-cols-[100px_1fr] gap-y-2 gap-x-1">
              <span className="text-slate-500">Họ và tên:</span>
              <span>{info.from_name}</span>

              <span className="text-slate-500">Số điện thoại:</span>
              <span>{info.from_phone}</span>

              <span className="text-slate-500">Địa chỉ gửi:</span>
              <span>{info.from_address}</span>
            </div>
          </section>
        </div>

        {/* 2. NGƯỜI NHẬN + CHI PHÍ */}
        <div className="grid md:grid-cols-2 gap-4">
          {/* Người nhận */}
          <section className="bg-white rounded-xl shadow-sm p-4 text-sm">
...
export default AdminGhnTrackingPage;
