...
export default function ProfilePage() {
  ...
  const [avatarError, setAvatarError] = React.useState<string>(""); // ❗ lỗi avatar
  ...
  async function onSave() {
    if (isGoogle) {
      toast.warn(
        "Tài khoản đăng nhập bằng Google không thể chỉnh sửa hồ sơ tại đây"
      );
      return;
    }
    if (!validate() || !user) return;

    // ❗ nếu avatar đang lỗi size thì không cho lưu
    if (avatarError) {
      ...
      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        {isGoogle && (
          <p className="mb-3 text-xs text-amber-600">
            Bạn đang đăng nhập bằng Google, các thông tin này chỉ đọc và không
            thể chỉnh sửa từ hệ thống.
          </p>
        )}
