...
export const AddressModal: React.FC<AddressModalProps> = ({
  ...
  async function handleSubmit() {
    ...
    const provinceId = provinces.find((p) => p.code === provinceCode)?.id ?? "";
    const districtId = districts.find((d) => d.code === districtCode)?.id ?? "";
    const wardId = wards.find((w) => w.code === wardCode)?.id ?? "";
